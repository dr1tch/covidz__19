        ...
        <!-- Delete Tag Modal -->
         ...
         <modal
            ...
            >
                <div class="card">
                    <div class="card-header">
                        <h4 class="font-weight-bold">Delete All Categories</h4>
                    </div>
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            getCategories() {
                this.callAPI('get', '/admin/categories/get')
                            .then(responce => this.categories = responce);
            },
            addCategory() {
            this.form.post('/admin/categories/create')
                .then(() => this.$modal.hide('add-category-modal'))
                .then(() => this.getCategories());
            },
             editCategory() {
                 console.log(this.editable.id);
                this.form.post(`/admin/categories/${this.editable.id}/update`)
                    .then(() => this.$modal.hide('edit-category-modal'))
                    .then(() => this.getCategories());
             },
             deleteCategory() {
                 this.form.post(`/admin/categories/${this.editable.id}/delete`)
                    .then(() => this.$modal.hide('delete-category-modal'))
                    .then(() => this.getCategories());
             },
             deleteAllCategories() {
                 this.form.post('/admin/categories/deleteAll')
